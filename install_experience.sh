...
# ── ExperiencePage.jsx ────────────────────────────────────────────────────────
cat > "$BASE/components/ExperiencePage.jsx" << 'EOF'
...
echo "✅ ExperiencePage.jsx créé"

# ── Patch App.jsx to add /experience route ────────────────────────────────────
APP="$BASE/App.jsx"

# Add lazy import after existing BookingFlow import
if ! grep -q "ExperiencePage" "$APP"; then
  sed -i '' "s/const BookingFlow = lazy/const ExperiencePage = lazy(() => import('.\/components\/ExperiencePage'))\nconst BookingFlow = lazy/" "$APP"
  echo "✅ ExperiencePage import ajouté dans App.jsx"
else
  echo "⚠️  ExperiencePage déjà importé dans App.jsx"
fi

# Add /experience route in the router logic
if ! grep -q "experience" "$APP"; then
  sed -i '' "s/const isBooking = path === '\/reserver'/const isExperience = path === '\/experience' || path === '\/experience\/'\n  const isBooking = path === '\/reserver'/" "$APP"

  sed -i '' "s/if (isBooking) {/if (isExperience) {\n    return (\n      <Suspense fallback={<BookingLoader \/>}>\n        <ExperiencePage \/>\n      <\/Suspense>\n    )\n  }\n\n  if (isBooking) {/" "$APP"

  echo "✅ Route \/experience ajoutée dans App.jsx"
else
  echo "⚠️  Route \/experience déjà présente dans App.jsx"
fi
...
echo "========================================"
echo ""
echo "Test: http://localhost:5175/experience"
