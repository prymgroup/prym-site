...
function Card({ icon, title, subtitle, desc, selectLabel, onClick, delay, isAR }) {
  return (
    <motion.button onClick={onClick}
      ...
      <div>
        <div style={{ fontFamily: '"Eurostile","Arial Narrow",sans-serif', fontSize: 11, letterSpacing: isAR ? '0.02em' : '0.2em', textTransform: isAR ? 'none' : 'uppercase', color: 'var(--c-text)', marginBottom: 2 }}>{title}</div>
        <div style={{ fontFamily: '"Eurostile","Arial Narrow",sans-serif', fontSize: 8, letterSpacing: isAR ? '0.02em' : '0.18em', textTransform: isAR ? 'none' : 'uppercase', color: 'var(--c-silver)', marginBottom: 8 }}>{subtitle}</div>
        <div style={{ fontSize: 11, color: 'var(--c-silver2)', lineHeight: 1.6, fontFamily: '"Nexa","Nexa Light",sans-serif', fontStyle: 'italic' }}>{desc}</div>
      </div>
      <div style={{ marginTop: 'auto', fontFamily: '"Eurostile","Arial Narrow",sans-serif', fontSize: 8, letterSpacing: isAR ? '0.02em' : '0.25em', textTransform: isAR ? 'none' : 'uppercase', color: 'var(--c-silver3)' }}>{selectLabel}</div>
    </motion.button>
  )
}
  ...
  return (
    <div style={{ width: '100%', flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'flex-start', justifyContent: 'center', padding: 'clamp(20px,5vw,48px) clamp(14px,4vw,32px)', gap: 10, boxSizing: 'border-box', direction: isAR ? 'rtl' : 'ltr' }}>
      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }}
        style={{ fontFamily: '"Eurostile","Arial Narrow",sans-serif', fontSize: 9, letterSpacing: isAR ? '0.02em' : '0.3em', textTransform: isAR ? 'none' : 'uppercase', color: 'var(--c-silver3)', marginBottom: 4 }}>
        {tb.eyebrow}
      </motion.p>
      <motion.h2 initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}
        style={{ fontFamily: '"Eurostile","Arial Narrow",sans-serif', fontSize: 'clamp(16px,5vw,28px)', letterSpacing: isAR ? '0.02em' : '0.1em', textTransform: isAR ? 'none' : 'uppercase', color: 'var(--c-text)', marginBottom: 20, fontWeight: 300 }}>
        {tb.h2}
      </motion.h2>
