...
cat > "$BASE/components/AProposPage.jsx" << 'EOF'
...
export default function AProposPage() {
  useEffect(() => {
    window.scrollTo(0, 0)
    document.title = 'À propos — PRYM Executive Transport'
  }, [])
...
echo "✅ AProposPage.jsx créé"

# ── Patch App.jsx ─────────────────────────────────────────────────────────────
APP="/Users/Apple/Desktop/prym site/prym/src/App.jsx"

if ! grep -q "AProposPage" "$APP"; then
  sed -i '' "s/const EntreprisesPage = lazy/const AProposPage = lazy(() => import('.\/components\/AProposPage'))\nconst EntreprisesPage = lazy/" "$APP"
  echo "✅ AProposPage import ajouté"
fi

if ! grep -q "isAPropos\|\/a-propos" "$APP"; then
  sed -i '' "s/const isEntreprises = /const isAPropos = path === '\/a-propos' || path === '\/a-propos\/'\n  const isEntreprises = /" "$APP"
  sed -i '' "s/if (isEntreprises) {/if (isAPropos) {\n    return (\n      <Suspense fallback={<BookingLoader \/>}>\n        <AProposPage \/>\n      <\/Suspense>\n    )\n  }\n\n  if (isEntreprises) {/" "$APP"
  echo "✅ Route \/a-propos ajoutée"
fi

echo ""
echo "========================================"
echo "✅ AProposPage installée"
echo "Test: http://localhost:5175/a-propos"
echo "========================================"
