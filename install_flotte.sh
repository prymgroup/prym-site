...
cat > "$BASE/components/FlottePage.jsx" << 'EOF'
...
export default function FlottePage() {
  ...
  useEffect(() => {
    window.scrollTo(0, 0)
    document.title = 'La Flotte — PRYM Executive Transport'
...
echo "✅ FlottePage.jsx créé"

# ── Patch App.jsx ─────────────────────────────────────────────────────────────
APP="/Users/Apple/Desktop/prym site/prym/src/App.jsx"

if ! grep -q "FlottePage" "$APP"; then
  sed -i '' "s/const ExperiencePage = lazy/const FlottePage = lazy(() => import('.\/components\/FlottePage'))\nconst ExperiencePage = lazy/" "$APP"
  echo "✅ FlottePage import ajouté"
else
  echo "⚠️  FlottePage déjà importé"
fi

if ! grep -q "isFlotte\|\/flotte" "$APP"; then
  sed -i '' "s/const isExperience = path/const isFlotte = path === '\/flotte' || path === '\/flotte\/'\n  const isExperience = path/" "$APP"
  sed -i '' "s/if (isExperience) {/if (isFlotte) {\n    return (\n      <Suspense fallback={<BookingLoader \/>}>\n        <FlottePage \/>\n      <\/Suspense>\n    )\n  }\n\n  if (isExperience) {/" "$APP"
  echo "✅ Route \/flotte ajoutée"
else
  echo "⚠️  Route \/flotte déjà présente"
fi

echo ""
echo "========================================"
echo "✅ FlottePage installée"
echo "Test: http://localhost:5175/flotte"
echo "========================================"
